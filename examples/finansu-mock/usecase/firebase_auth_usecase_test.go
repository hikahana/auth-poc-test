...
func TestFirebaseSignInLinksExistingUserByVerifiedEmail(t *testing.T) {
	...
	f.platformSays("approved", "firebase-uid-1", "member@example.com", true)
...
func TestFirebaseSignInFindsLinkedUserBySubAfterEmailChange(t *testing.T) {
	...
	f.platformSays("approved", "firebase-uid-1", "renamed@example.com", true)
	token, err := f.firebase.SignIn(ctx, "id-token")
	if err != nil {
...
func TestFirebaseSignInRejections(t *testing.T) {
	tests := []struct {
		...
		{
			name: "not approved by whitelist",
			setup: func(f *fixture, t *testing.T) {
				f.existingUser(t, "member@example.com", 1)
				f.platformSays("pending", "firebase-uid-1", "member@example.com", true)
			},
			want: ErrNotApproved,
		},
		{
			...
			setup: func(f *fixture, t *testing.T) {
				f.existingUser(t, "member@example.com", 1)
				f.platformSays("approved", "firebase-uid-1", "member@example.com", false)
			},
			want: ErrEmailNotVerified,
		},
		{
			name: "approved but no FinanSu account",
			setup: func(f *fixture, t *testing.T) {
				f.platformSays("approved", "firebase-uid-1", "stranger@example.com", true)
			},
			want: ErrNoAccount,
		},
		{
			...
			setup: func(f *fixture, t *testing.T) {
				f.existingUser(t, "member@example.com", 1)
				f.platformSays("approved", "someone-else", "member@example.com", true)
